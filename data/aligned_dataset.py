...
def channel_1toN(img, num_channel):
    ...
    for i in range(num_channel):
        T[i] = T[i] + i
        layer = T[i] - img
        T[i] = torch.from_numpy(np.logical_not(
            np.logical_xor(layer.numpy(), mask.numpy())).astype(int))
    return T.float()
...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.dir_A = os.path.join(
            opt.dataroot, opt.phase + '_' + opt.dataset + '_A')
        self.A_paths = sorted(make_dataset(self.dir_A))
        self.dir_B = os.path.join(
            opt.dataroot, opt.phase + '_' + opt.dataset + '_B')
    ...
    def __getitem__(self, index):
        ...
        A_S = A.resize((int(self.opt.loadSize * 0.75),
        ...
        if self.opt.loadSize > self.opt.fineSize:
            if random.random() < 0.4:
                ...
                w = min(int(round(math.sqrt(target_area * aspect_ratio))),
                        self.opt.loadSize)
                h = min(int(round(math.sqrt(target_area / aspect_ratio))),
                        self.opt.loadSize)
                ...
                A = A.resize(
                    (self.opt.fineSize, self.opt.fineSize), Image.LANCZOS)
                B = B.resize(
                    (self.opt.fineSize, self.opt.fineSize), Image.NEAREST)

            elif 0.4 < random.random() < 0.95:
                w_offset = random.randint(
                    0, max(0, A.size[1] - self.opt.fineSize - 1))
                h_offset = random.randint(
                    0, max(0, A.size[0] - self.opt.fineSize - 1))
                A = A.crop((w_offset, h_offset, w_offset +
                            self.opt.fineSize, h_offset + self.opt.fineSize))
                B = B.crop((w_offset, h_offset, w_offset +
                            self.opt.fineSize, h_offset + self.opt.fineSize))
        ...
        # multi channel float tensor for thumbnail
        B_attribute_GAN = channel_1toN(B_attribute_L1, self.opt.output_nc)
