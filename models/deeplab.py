...
class D_ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, input_size):
        ...
        self.maxpool = nn.MaxPool2d(
            kernel_size=3, stride=2, padding=1)  # change
        self.layer1 = self._make_layer(
            block, 64, layers[0], stride=1, dilation=[1])
        self.layer2 = self._make_layer(
            block, 128, layers[1], stride=2, dilation=[1])
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=2, dilation=[1])
        self.layer4 = self._make_layer(
            block, 512, layers[3], stride=1, dilation=[1, 1, 1], rate=2)
        self.layer5 = self._make_pred_layer(
            D_Classifier_Module, [2, 4, 6], [2, 4, 6], num_classes)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=[1], rate=1):
        ...
        layers.append(block(self.inplanes, planes, stride,
                            dilation=rate * dilation[0], downsample=downsample))
